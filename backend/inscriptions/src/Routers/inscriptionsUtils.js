...
function calculateTotalCost(eventsInfo, competition, athlete, athleteInscriptions = null) {
    let totalCost = 0;
    if (!(competition.freeClub && athlete.club == competition.club)) {
        for (let i = 0; i < eventsInfo.length; i++) {
            if (!athleteInscriptions || !athleteInscriptions.some((inscription) => inscription.event == eventsInfo[i].pseudoName)) {
                totalCost += eventsInfo[i].cost;
            }
        }
    }
    return totalCost;
