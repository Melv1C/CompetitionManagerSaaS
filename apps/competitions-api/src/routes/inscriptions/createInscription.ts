...
router.post(
    ...
    async (req: CustomRequest, res) => {
        try {
            ...
            if (
                !isAdmin &&
                (parsedCompetition.startInscriptionDate > new Date() ||
                    parsedCompetition.endInscriptionDate < new Date())
            ) {
            ...
            for (const athInscriptions of inscriptionsData) {
                try {
                    ...
                    const athlete = await findAthleteWithLicense(
                        athInscriptions.athleteLicense,
                        z.array(Athlete$).parse(competition.oneDayAthletes)
                    );
                    const cat = getCategoryAbbr(
                        athlete.birthdate,
                        athlete.gender,
                        parsedCompetition.date
                    );

                    // Check if the athlete club is allowed to inscribe
                    if (
                        !isAdmin &&
                        parsedCompetition.allowedClubs.length > 0 &&
                        !parsedCompetition.allowedClubs
                            .map((c) => c.id)
                        ...
                        res.status(400).send(
                            'Club ' +
                                athlete.club.abbr +
                                ' is not allowed to inscribe in this competition'
                        );
                        return;
                    }
                    ...
                    if (
                        !isAdmin &&
                        parsedCompetition.maxEventByAthlete &&
                        athInscriptions.inscriptions.length >
                            parsedCompetition.maxEventByAthlete
                    ) {
                        res.status(400).send(
                            'Athlete ' +
                                athInscriptions.athleteLicense +
                                ' is inscribed in too many events'
                        );
                        return;
                    }
                    ...
                    for (const inscription of athInscriptions.inscriptions) {
                        ...
                        const event = parsedCompetition.events.find(
                            (e) => e.eid === inscription.competitionEventEid
                        );
                        if (!event) {
                            res.status(404).send(
                                'Event ' +
                                    inscription.competitionEventEid +
                                    ' not found'
                            );
                            return;
                        }
                        ...
                        if (!event.categories.some((c) => c.abbr === cat)) {
                            res.status(400).send(
                                'Athlete ' +
                                    athInscriptions.athleteLicense +
                                    ' is not in the right category for event ' +
                                    inscription.competitionEventEid +
                                    ' (' +
                                    cat +
                                    ')'
                        ...
                        const nbParticipants = competition.inscriptions.filter(
                            (i) =>
                                i.isDeleted === false &&
                                i.status === InscriptionStatus.ACCEPTED &&
                                i.competitionEventId === event.id &&
                                i.athlete.license !==
                                    athInscriptions.athleteLicense // Don't count the current athlete
                        ).length;
                        // Check if event is full
                        if (
                            !isAdmin &&
                            event.place &&
                            nbParticipants >= event.place
                        ) {
                            res.status(400).send(
                                'Event ' +
                                    inscription.competitionEventEid +
            ...
            const { alreadyPaid, fees, totalToPay, totalCost } = (
                await Promise.all(
                    inscriptionsData.map(
                        async ({ athleteLicense, inscriptions }) => {
                            ...
                            return getCostsInfo(
                                parsedCompetition,
                                athlete,
                                inscriptions.map((i) => i.competitionEventEid),
                                Inscription$.array()
                                    .parse(competition.inscriptions)
                                    .filter((i) => i.user.id === req.user!.id)
                            );
                        }
                    )
                )
            ).reduce(
                (acc, { totalCost, alreadyPaid, fees, totalToPay }) => {
            ...
            const nbOfInscriptionsByEvent = inscriptionsData.reduce(
                (acc, { inscriptions }) => {
                    inscriptions.forEach(({ competitionEventEid }) => {
                        acc[competitionEventEid] =
                            (acc[competitionEventEid] || 0) + 1;
                    });
                    return acc;
                },
            ...
            if (totalToPay > 0) {
                try {
                    const inscriptions = inscriptionsData
                        .map(({ athleteLicense, inscriptions }) => {
                    ...
                    const lineItems = [
                        ...Object.entries(nbOfInscriptionsByEvent).map(
                            ([eid, quantity]) => {
                                const event = parsedCompetition.events.find(
                                    (e) => e.eid === eid
                                );
                    ...
                    const session = await createCheckoutSession({
                        ...
                        custom_text: {
                            after_submit: {
                                message: req.t('stripe.only30Minutes'),
                            },
                        },
                    });
