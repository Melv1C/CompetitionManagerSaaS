...
export const Success: React.FC<SuccessProps> = ({ handleResart }) => {
    ...
    const translationBaseKey = searchParams.has('isPending')
        ? 'competition:pending'
        : 'competition:success';
    ...
    return (
        ...
        >
            <CardContent>
                <Alert
                    severity={
                        searchParams.has('isPending') ? 'info' : 'success'
                    }
                >
                    <AlertTitle>{t(`${translationBaseKey}.title`)}</AlertTitle>
