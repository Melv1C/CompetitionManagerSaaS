...
export const CategoriesPie = () => {
    ...
    const categories = useMemo(() => new Set(inscriptions.map((i) => getCategoryAbbr(i.athlete.birthdate, i.athlete.gender, competition.date))), [inscriptions, competition]);
            ...
            <PieChart 
                series={[{
                    data: Array.from(categories).map((category) => ({
                        id: category,
                        value: inscriptions.filter((i) => getCategoryAbbr(i.athlete.birthdate, i.athlete.gender, competition.date) === category).length,
                        label: category
                    })),
                    arcLabel: 'label',
                    arcLabelMinAngle: 30,
