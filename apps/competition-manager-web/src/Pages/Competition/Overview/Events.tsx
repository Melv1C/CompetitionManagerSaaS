            ...
            competition.events
                .reduce((acc, event) => {
                    event.categories.forEach((category) => {
                        if (
                            !acc
                                .map((c) => getFullBaseCategory(c))
                                .includes(getFullBaseCategory(category))
                        ) {
                            acc.push(category);
                        }
                    });
                ...
                .reduce((acc, event) => {
                    event.categories.forEach((category) => {
                        ...
                        if (
                            !acc[getFullBaseCategory(category)]
                                .map((e) => e.abbr)
                                .includes(event.event.abbr)
                        ) {
                            acc[getFullBaseCategory(category)].push(
                                event.event
                            );
                        }
                    });
                    return acc;
                }, {} as Record<Category['abbr'], Event[]>),
        [competition.events]
    );
    ...
    return (
        <>
            {[Gender.F, Gender.M].map((gender) => {
                if (
                    allBaseCategories.filter((c) => c.gender === gender)
                        .length > 0
                ) {
                    return (
                        <Card
