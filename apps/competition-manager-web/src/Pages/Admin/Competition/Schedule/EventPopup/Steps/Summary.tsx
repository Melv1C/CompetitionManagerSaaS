...
export const Summary: React.FC<SummaryProps> = ({ handleBack, handleNext }) => {
    ...
    const onCreate = async () => {
        ...
        const createdCompetitionEvent = await createCompetitionEvent(
            competition.eid,
            createCompetitionEventData
        );

        const createdChildren = await Promise.all(
            competitionEventData.children.map(async (child) => {
                const createSubCompetitionEventData =
                    CreateCompetitionEvent$.parse({
                        ...child,
                        eventId: child.event!.id,
                        place: competitionEventData.place,
                        cost: 0,
                        categoriesId: competitionEventData.categories.map(
                            (c) => c.id
                        ),
                        parentEid: createdCompetitionEvent.eid,
                    });
                return await createCompetitionEvent(
                    competition.eid,
                    createSubCompetitionEventData
                );
            })
        );
    ...
    const onUpdate = async () => {
        ...
        const updatedCompetitionEvent = await updateCompetitionEvent(
            competition.eid,
            competitionEventData.eid!,
            updateCompetitionEventData
        );

        const updatedChildren = await Promise.all(
            competitionEventData.children.map(async (child) => {
                const updateSubCompetitionEventData =
                    UpdateCompetitionEvent$.parse({
                        ...child,
                        eventId: child.event!.id,
                        place: competitionEventData.place,
                        cost: 0,
                ...
                return await updateCompetitionEvent(
                    competition.eid,
                    child.eid!,
                    updateSubCompetitionEventData
                );
            })
        );
        ...
        const updatedEvents = [
            ...competition.events
                .filter((e) => e.eid !== updatedCompetitionEvent.eid)
                .filter(
                    (e) => !updatedChildren.map((c) => c.eid).includes(e.eid)
                ),
        ...
        <Box>
            ...
            >
                ...
                <FormControl>
                    <TextField
                        ...
                        value={
                            competitionEventData.schedule.toLocaleDateString(
                                'fr'
                            ) +
                            ' ' +
                            competitionEventData.schedule.toLocaleTimeString(
                                'fr',
                ...
                <FormControl>
                    <TextField
                        ...
                        value={
                            competitionEventData.cost === 0
                                ? t('glossary:free')
                                : competitionEventData.cost.toString()
                        }
                        slotProps={{
                            input: { readOnly: true },
                        }}
            ...
            <StepperButtons
                buttons={[
                    ...
                    isCreate
                        ? { label: t('buttons:create'), onClick: onCreate }
                        : { label: t('buttons:modify'), onClick: onUpdate },
                ]}
            />
        </Box>
    );
