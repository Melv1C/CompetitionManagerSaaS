...
import { Box, Button, Card, CardHeader, Chip, Dialog, DialogActions, DialogContent, DialogTitle, List, ListItem, ListItemText, Typography } from "@mui/material"
...
export const Popup: React.FC<PopupProps> = ({ 
    ...
    const [inscriptionsState, setInscriptionsState] = useState<Inscription[]>(inscriptions);

    const isModified = useMemo(() => {
        return JSON.stringify(inscriptions) !== JSON.stringify(inscriptionsState);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
        ...
        if (event.altKey && event.key === 'c') {
            setInscriptionsState(inscriptionsState.map(i => ({
                ...i,
    ...
    return (
        ...
        >
            <DialogTitle>
                {t('glossary:confirmation')}
            </DialogTitle>
            <DialogContent>
                <Card elevation={0}>
                    <CardHeader 
                        title={<Box display="flex" alignItems="center" gap={2}>
                            ...
                            <ListItem 
                                ...
                                secondaryAction={
                                    <Box display="flex" gap={1}>
                                        <Chip 
                                            label={t(`enums:inscriptionStatus.confirmed`)}
                                            color="success"
                                            variant={i.status === InscriptionStatus.CONFIRMED ? 'filled' : 'outlined'}
                                            sx={{ pl: 1 }}
                                            icon={<FontAwesomeIcon icon={faCheck}/>}
                                            onClick={() => setInscriptionsState(inscriptionsState.map(inscription => {
                                        ...
                                        />
                                        <Chip 
                                            label={t(`enums:inscriptionStatus.removed`)}
                                            color="error"
                                            variant={i.status === InscriptionStatus.REMOVED ? 'filled' : 'outlined'}
                                            sx={{ pl: 1 }}
                    ...
                    <Box display="flex" justifyContent="flex-end" p={2} gap={1}>
                        <Chip
                            label={<Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                                <Typography variant="body2">
                                    {t('adminCompetition:confirmAll')}
                                </Typography>
                                <Box sx={{ borderRadius: '4px', border: '2px solid', borderColor: 'divider', p: '1px' }}>
                                    Alt + C
                                </Box> 
                            </Box>}
                            color="success"
